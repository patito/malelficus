...
#define PRINT_C fprintf
#define PRINT_FILEADDR PRINT_C(fd, "/* %06x */", file_addr);
...
#define PRINT_WORD(value, desc) do { PRINT_FILEADDR PRINT_C(fd, "\t%d,\t/* %s */\n", value, desc); file_addr += sizeof(value); } while(0)
#define PRINT_WORD_END(value, desc) do { PRINT_FILEADDR PRINT_C(fd, "\t%d\t/* %s */\n", value, desc); file_addr += sizeof(value); } while(0)
#define PRINT_HALF(value, desc) PRINT_WORD(value, desc)
#define PRINT_ADDR(value, desc) do { PRINT_FILEADDR PRINT_C(fd, "\t0x%x,\t/* %s */\n", value, desc); file_addr += sizeof(value); } while(0)
#define PRINT_ADDR_END(value, desc) do { PRINT_FILEADDR PRINT_C(fd, "\t0x%x\t/* %s */\n", value, desc); file_addr += sizeof(value); } while(0)
...
_u8 reverse_elf2c(malelf_object* elf, FILE* fd) {
  ...
  PRINT_C(fd, TOP_TEMPLATE);

  PRINT_C(fd, "Elf32_Ehdr elf_header = {\n");
  ...
  PRINT_C(fd, "const unsigned char segments[] = \n");
  ...
  PRINT_C(fd, "\n");
  PRINT_C(fd, SELF_GENERATE_SRC, bname, bname);
